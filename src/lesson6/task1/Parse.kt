...
fun bestHighJump(jumps: String): Int {
    var count = 0
    return if (Regex("""[^\d\s-+%]""").containsMatchIn(jumps)) -1
    else {
        var bestJump = -1
        val parts = jumps.split(" ")
        for (i in parts) {
            count++
            if (Regex("""\d""").containsMatchIn(i)) {
                val nextSymbol = parts[count]
                if ((i.toInt() > bestJump) && (nextSymbol == "+"))
                    bestJump = i.toInt()
            }
        }
        bestJump
...
fun mostExpensive(description: String): String {
    var mostExpensivePrice = -1.0
    var mostExpensiveName = ""
    val listGoods = description.split("; ")
    for (i in listGoods) {
        val parts = i.split(" ")
...
fun fromRoman(roman: String): Int {
    ...
    while (sizeRoman != 0) {
        for (i in 0 until roman.length) {
            if (j == i) {
                var currentDigitString = roman[i].toString()
                if (!digits.containsKey(currentDigitString)) return -1
                if (roman.length == 1) return digits[currentDigitString] ?: error("")
                if (j + 1 != roman.length) {
                    while (digits.containsKey(roman[j] + roman[j + 1].toString())) {
                        currentDigitString += roman[j + 1].toString()
                        sizeRoman--
                        j++
                        if (j + 1 == roman.length) break
                    }
                }
                result += digits[currentDigitString] ?: error("")
                sizeRoman--
                j++
            }
        }
